...
| `?login` | `controllers/system/login.php`, `views/Login.tpl.html` | `GET,POST /?login`, `siteService.login` | Partial, changed | v2 now prefers persistent email/password users when present and keeps the configured env-admin fallback for bootstrapping/migration compatibility. Sessions are still in-memory. |
...
| `?profile` | `controllers/system/profile.php`, `views/Profile.tpl.html` | `GET,POST /?profile`, `siteService.profile` | Partial | v2 shows session info and supports CSRF-protected password changes for persistent users. Env-admin profile updates remain intentionally limited. |
| `?recover` and `?recover=<code>` | `controllers/system/recover.php`, `views/RecoverPassword.tpl.html`, `views/ChangePassword.tpl.html` | `GET,POST /?recover`, `siteService.recover` | Foundation only | Recovery token generation/storage uses hashed expiring tokens, but HTTP email delivery/reset flow remains `501` until mail delivery is designed. |
| `?join` | `controllers/system/join.php`, `views/Join.tpl.html` | `GET,POST /?join`, `siteService.join` | Partial | First-admin bootstrap creates one persistent Superuser-equivalent admin only while no persistent users exist. General public registration remains planned. |
...
| `user`: identity, email, `password`, profile, quotas, `autograb`, `domaintograb`, status | `User`, `Credential`, optional `UserPreference` | Foundation only | A file-backed auth store persists `Data.User`-aligned users under the archive root with versioned salted HMAC-SHA256/PBKDF2-like password hashes. Migrate MD5 password values only through forced reset or one-time verifier; never keep MD5 as active auth. |
| `group`: `owner_id`, `name`, `title`, `comment`, `date`, `status`, `domain` | `Group` | Foundation only | First-admin bootstrap creates the Superuser group. Full group management and authorization checks are still planned. |
| `user_group` | `UserGroup` | Foundation only | First-admin bootstrap creates the initial user/group link. Broader many-to-many membership and v1 `AmIInGroup` parity remain planned. |
